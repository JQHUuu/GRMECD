...
def __partial_entropy_a_proba(proba):
    if proba == 0:
        return 0
    return -proba * 1.0 * math.log(proba, logBase)


def __cover_entropy(cover, allNodes):  # cover is a list of set, no com ID
    allEntr = []
    for com in cover:
        fractionIn = len(com) * 1.0 / len(allNodes)
...
def __com_pair_conditional_entropy(cl, clKnown, allNodes):  # cl1,cl2, snapshot_communities (set of nodes)
    ...
    nbNodes = len(allNodes) * 1.0  ###zhuyi   zhengchu
...
def __cover_conditional_entropy(cover, coverRef, allNodes, normalized=False):  # cover and coverRef and list of set
    ...
    for com in cover:
        ...
        HXY_part = bestMatch[1] * 1.0
        if normalized:
            HX = __partial_entropy_a_proba(len(com) * 1.0 / len(allNodes)) + __partial_entropy_a_proba(
                (len(allNodes) - len(com)) * 1.0 / len(allNodes))
            if HX == 0:
                HXY_part = 1.0
            else:
...
def onmi(cover, coverRef, allNodes=None, variant="LFK"):  # cover and coverRef should be list of set, no community ID
    ...
    if variant == "LFK":
        HXY = __cover_conditional_entropy(cover, coverRef, allNodes, normalized=True) * 1.0
        HYX = __cover_conditional_entropy(coverRef, cover, allNodes, normalized=True) * 1.0
    else:
        HXY = __cover_conditional_entropy(cover, coverRef, allNodes) * 1.0
        HYX = __cover_conditional_entropy(coverRef, cover, allNodes) * 1.0

    HX = __cover_entropy(cover, allNodes) * 1.0
    HY = __cover_entropy(coverRef, allNodes) * 1.0
